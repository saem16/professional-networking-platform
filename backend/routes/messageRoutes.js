...
router.post('/conversations/direct', authMiddleware, async (req, res) => {
  try {
    ...
    console.log('Creating direct conversation:', { userId, recipientId });
...
router.post('/conversations/:conversationId/messages', authMiddleware, upload.single('attachments'), async (req, res) => {
    try {
      const userId = req.userId;
      const { conversationId } = req.params;
      ...
      console.log('Send message - req.file:', req.file);
